...
const getAllBesoin = async (req, res)=> {
  const query = `SELECT BESOIN.* , ARTICLE.*, AGENT.*, CATEGORIE.*,DIVISION.* FROM ((((BESOIN
    INNER JOIN ARTICLE ON BESOIN.FORMULE = ARTICLE.FORMULE)
    INNER JOIN AGENT ON BESOIN.MATRICULE = AGENT.MATRICULE)
    INNER JOIN CATEGORIE ON ARTICLE.ID_CAT=CATEGORIE.ID_CAT)
...
const addHistorique = async(req,res,HIST_PREV, HIST_BES, ANNEE)=>{
  console.log("annee : ",ANNEE)
  const query = ` INSERT INTO HISTORIQUE (NUM_HIST, HIST_PREV, HIST_BES, ANNEE) VALUES(NUM_HIST.nextval, :HIST_PREV, :HIST_BES, :ANNEE) `
  try {
      const connection = await getConnection();
      const result = await connection.execute(query,[HIST_PREV, HIST_BES, ANNEE]);
      res.json(result.rows);
      connection.commit()
      await connection.close()
  } catch (error) {
